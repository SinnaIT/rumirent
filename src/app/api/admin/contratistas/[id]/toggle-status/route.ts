...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const updatedContratista = await prisma.user.update({
      where: { id },
      data: {
        activo: !existingContratista.activo
      },
      select: {
        id: true,
        email: true,
    ...
    return NextResponse.json({
      success: true,
      message: `Contratista ${updatedContratista.activo ? 'activado' : 'desactivado'} exitosamente`,
      contratista: updatedContratista
    })

  } catch (error) {
    console.error('Error al cambiar estado del contratista:', error)
    return NextResponse.json(
