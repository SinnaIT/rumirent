...
export async function GET(request: NextRequest) {
  try {
    ...
    const totales = {
      ...
      totalComision: leads.reduce((sum, lead) => sum + lead.comision, 0),
      ...
      totalPendiente: leads.filter(l => !l.conciliado).reduce((sum, lead) => sum + lead.comision, 0),
      leadsConciliados: leads.filter(l => l.conciliado).length,
      leadsPendientes: leads.filter(l => !l.conciliado).length,
